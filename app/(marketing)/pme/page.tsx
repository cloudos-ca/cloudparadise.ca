 ...
 *
 * Conséquence pratique : le miroir anglais sera **ce fichier**, avec `LANG` à
 * `"en"` et les chemins inversés. Rien d'autre à traduire ici.
 */

/** Locale servie par cette route. Le miroir `/en/pme` posera `"en"`. */
const LANG: Lang = "fr";
...
export const metadata: Metadata = {
  ...
  // Le chemin EN est celui de la page courante tant que `/en/pme` n'existe
  // pas : pour la locale `fr`, seul le premier argument est lu (il devient le
  // canonical), et inventer une URL anglaise ici la ferait fuiter le jour où
  // quelqu'un lirait cet appel de travers.
  alternates: alternatesBilingues("/pme", "/pme", "fr"),
...
export default function PmePage() {
  return (
    <>
      {/* Pas de `hreflang="en"` : la page anglaise n'existe pas encore, et
          déclarer une paire vers une 404 ferait douter des paires valides
          déclarées ailleurs (voir HreflangLinks). */}
      <HreflangLinks fr="/pme" />
      <BreadcrumbJsonLd
        items={[
